...
export default function IssueDetail() {
  ...
  const pausableStates = new Set(['queued', 'refining', 'approved', 'building', 'in_review', 'addressing_feedback', 'fixing_checks', 'qa', 'qa_fix'])
    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '24px', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      {((issue.build_activity?.length ?? 0) > 0 || streamEvents.length > 0) && (
        <section style={{ marginBottom: '24px' }}>
          ...
          <BuildLog activities={[...(issue.build_activity ?? []), ...streamEvents]} />
        </section>
      )}

      {/* Timeline — API already excludes build_event entries */}
      <section>
        <h2 style={{ fontSize: '16px', fontWeight: 600, color: '#374151', marginBottom: '12px' }}>
          Timeline
        </h2>
