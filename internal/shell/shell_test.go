...
func TestRunWithStdinStreaming_PipesInputAndReturnsOutput(t *testing.T) {
	r := &Runner{}
	out, err := r.RunWithStdinStreaming(context.Background(), "hello streaming", "cat")
	if err != nil {
		t.Fatalf("RunWithStdinStreaming failed: %v", err)
	}
	if got := strings.TrimSpace(out); got != "hello streaming" {
		t.Errorf("output = %q, want %q", got, "hello streaming")
	}
}

func TestRunWithStdinStreaming_NonZeroExit_ReturnsExitError(t *testing.T) {
	r := &Runner{}
	_, err := r.RunWithStdinStreaming(context.Background(), "", "sh", "-c", "exit 7")
	if err == nil {
		t.Fatal("expected error, got nil")
	}
	var exitErr *ExitError
...
func TestRunWithStdinStreaming_WorkingDirectory(t *testing.T) {
	r := &Runner{Dir: "/tmp"}
	out, err := r.RunWithStdinStreaming(context.Background(), "", "pwd")
	if err != nil {
		t.Fatalf("RunWithStdinStreaming failed: %v", err)
	}
	got := strings.TrimSpace(out)
